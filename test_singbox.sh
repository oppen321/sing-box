...
if command -v go &> /dev/null; then
  ...
  if [[ "$INSTALLED_GO_VERSION" == "$GO_VERSION" ]]; then
    echo "Go 版本 $GO_VERSION 已经安装，跳过下载。"
  else
    echo "Go 版本不匹配，当前版本: $INSTALLED_GO_VERSION, 需要版本: $GO_VERSION。"
  ...
  echo "Go 未安装。正在安装 Go 版本 $GO_VERSION ($GO_ARCH)..."
  wget -P $DEST_DIR "$BASE_URL/go$GO_VERSION.linux-$GO_ARCH.tar.gz"
  tar -zxf "$DEST_DIR/go$GO_VERSION.linux-$GO_ARCH.tar.gz" -C $INSTALL_DIR
fi
...
if systemctl list-units --type=service --state=active | grep -q "sing-box.service"; then
  echo "sing-box 服务已安装。"
  read -p "选择操作：1.重启服务 2.停止服务 3.删除服务 (请输入 1/2/3): " SERVICE_OPTION
